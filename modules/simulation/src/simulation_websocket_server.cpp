...
public:
    ...
    simulation_websocket_server(int port, std::string aircraft_path, float deltatime = 1, int tick_time = 2,
                                int hil_type = 0) :
        ...
        switch (hil_type) {
            case HIL_NONE:
                break;
            case HIL_DJI:
                hil_adapter = (simulation_hil_adapter *) new simulation_dji_a3_adapter(simulatorAircraft);
                hil_adapter->main_thread();
                break;
            case HIL_PIXHAWK:
                hil_adapter = (simulation_hil_adapter *) new simulation_pixhawk_adapter(simulatorAircraft);
                hil_adapter->main_thread();
                break;
        }
    }
...
int main(int argc, char **argv) {
    ...
    simulation_websocket_server server(9093, path, 0.5, 5, HIL_PIXHAWK);
